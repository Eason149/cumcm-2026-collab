"""Deterministic mixed-source simulator for Problem B, Question 4."""
...
from question3_strategy import ClearReply, MeasureReply, bearing_deg, distance_m
...
class MixedSource:
    ...
    def covers(self, point: Point, tolerance: float = 1e-9) -> bool:
        """Whether ``point`` lies in the source's closed emission semicircle."""

        if self.direction_deg is None:
            return True
        angle = self.direction_deg * pi / 180.0
        return (
            (point.x - self.position.x) * cos(angle)
            + (point.y - self.position.y) * sin(angle)
            >= -tolerance
        )


class LocalMixedSimulator:
    """Implements the official motion, channel, sensing, and clearing rules."""

    def __init__(self, sources: list[MixedSource], seed: int = 0) -> None:
    ...
    def measure(self, position: Point, channel: int) -> MeasureReply:
        ...
        if source is None or source.cleared:
            ...
            if separation > source.reception_radius_m + 1e-9 or not source.covers(position):
                result = "no_signal"
...
def random_case(
    seed: int,
    minimum_sources: int = 10,
    maximum_sources: int = 16,
) -> list[MixedSource]:
    """Generate one reproducible case containing both source types."""

    rng = Random(seed)
    count = rng.randint(minimum_sources, maximum_sources)
    directional_count = rng.randint(1, count - 1)
    directional_indices = set(rng.sample(range(count), directional_count))
    ...
    for index, channel in enumerate(channels):
        ...
        sources.append(
            MixedSource(
                ...
                direction_deg=(rng.uniform(0.0, 360.0) if index in directional_indices else None),
            )
        )
    return sources
