...
PROFILE_ORDER = ("backbone21", "certified", "balanced", "rapid", "turbo", "fast", "sprint")
PROFILE_NOTES = {
    "backbone21": "21-station exact-route backbone; v4 local package strategy adapter.",
...
def run_profile_cases(profile: str, case_count: int) -> dict[str, object]:
    ...
    for seed in range(case_count):
        ...
        result = (
            run_backbone21(simulator)
            if profile == "backbone21"
            else MixedDirectionalSearch(circle_sides=120, survey_profile=profile).run(simulator)
        )
        records.append(
            {
                "seed": seed,
...
def build_report(case_count: int = 30, risk_samples: int = 500_000) -> dict[str, object]:
    ...
    for profile in PROFILE_ORDER:
        ...
        profiles[profile] = {
            "station_count": len(backbone21_stations() if profile == "backbone21" else survey_stations_for_profile(profile)),
    ...
    return {
        ...
        "recommended_formal_profile": "backbone21",
        "profiles": profiles,
    }
