...
import math
import numpy as np
from base import (ERROR,DirectionBelief,bearing_halfplanes,clip_polygon,exclude_disk,
    initial_polygon,minimum_circle,open_tour,optical_cover,stations_and_triangles)
from base import probe_pair
...
def solve(client,route='sequential',share=False,directional=False,angle_bin=4.,trial_radius=40.,
    ...
    def crossbar_plan(c):
        ...
        for ref in refs:
            try:
                if conditional_range:points,cert=conditional_pair(d['polygon'],ref['position'],ref['response']['svd_deg'],client.position,crossbar_offset,crossbar_fraction)
    ...
    while unknown or known:
        ...
        if not known or (search_first and active_sites):
            ...
            i=pool[order[0]] if route=='joint' else min(pool,key=lambda i:math.dist(client.position,sites[i]))
