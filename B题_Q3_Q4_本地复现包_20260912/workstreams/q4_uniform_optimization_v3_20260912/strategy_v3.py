...
import math
import numpy as np
from bridge import (ERROR,DirectionBelief,bearing_halfplanes,clip_polygon,exclude_disk,
    initial_polygon,minimum_circle,open_tour,optical_cover,stations_and_triangles)
from bridge import probe_pair
...
def solve(client,route='sequential',share=False,directional=False,angle_bin=4.,trial_radius=40.,
          ...
          near_clear_distance=50.,near_clear_radius=20.):
    ...
    sweep_start=None;sweep_direction=1;discovery_stops=[]
    ...
    def opportunistic_scan():
        ...
        for i in indices[:3]:
            if math.dist(pos,sites[i])>650:continue
            candidate=sites.copy();candidate[i]=pos;active=sorted(set(range(len(sites)))-retired)
            cert=triangle_certificate(candidate[active])
            ...
            if drop_sites:
                for j in sorted(unused):
                    active=sorted(set(range(len(sites)))-retired-{j});other=triangle_certificate(sites[active])
                    if other['passed']:
    ...
    def clear_at(c,point,kind,certificate=None):
        ...
        if reply['clear_result']=='success':
            cleared.append({'channel':c,'point':list(point),'bound_source':'prior_geometry' if bound<=20 else 'successful_clear_response',
                'prior_bound_m':bound,'virtual_after_s':client.virtual});known.pop(c);opportunistic_scan();return True
    ...
    while unknown or known:
        ...
        if route=='sequential':
            ...
            if route_mode=='backbone':
                ...
                order=insert_sources(client.position,points,[site_map[i] for i in backbone if i in site_map],[j for j,(kind,_) in enumerate(labels) if kind=='source'])
    ...
    return {'strategy':'q4_local_prototype','cleared_sources':cleared,'cleared_count':len(cleared),'absent_channels':sorted(absent),
        ...
        'discovery_upper_bound_stops':discovery_stops}
